...
# Force default experimental input to repo data/raw as requested.
...
# Keep de novo input optional from data/raw: if file is missing, import step is skipped.
...
def main() -> None:
	...
	with KnowledgeGraphBuilder() as builder:
		...
		try:
			...
			if label_col not in df_experimental.columns:
				raise KeyError(f"Label column '{label_col}' not found in {EXPERIMENTAL_CSV}")
			...
			print(f"File not found: {EXPERIMENTAL_CSV}")
		except Exception as e:
			print(f"Failed to build train-only KG: {e}")
			raise
